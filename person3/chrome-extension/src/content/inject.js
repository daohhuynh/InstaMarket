  ...
  async function injectTweetLayer(tweet) {
    ...
    layer.innerHTML = `
      <div class="im-market-question">
        ...
        <span class="im-match-confidence">Â· ${match.confidence}% ${match.source === 'aws-bedrock' ? 'AI' : 'parser'} match</span>
      </div>
    ...
    // Bet buttons â†’ instant bet + show markets sidebar
    layer.querySelectorAll('.im-bet-yes, .im-bet-no').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const side = btn.dataset.side;
        const mId = btn.dataset.market;
        showToast(`Bet placed: ${side} on "${market.question.slice(0, 40)}â€¦" âœ“`);
    ...
    layer.querySelector('.im-save-btn').addEventListener('click', e => {
      ...
      if (typeof window.saveMarketForLater === 'function') {
        ...
        showToast(saved ? `Saved: "${market.question.slice(0, 40)}â€¦" âœ“` : 'Already saved.');
        return;
      }
      showToast(`Saved: "${market.question.slice(0, 40)}â€¦" âœ“`);
    });
